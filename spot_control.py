...
def main():
    ...
    wasd_interface._drive_cmd(ord(' '))
    time.sleep(10)
    wasd_interface._request_power_on()
    print(wasd_interface._power_state)
    print('powering on')
    time.sleep(5)
    wasd_interface._drive_cmd(ord('p'))
    time.sleep(2)

    while True:
        text = listen()
        if text:
            my_cmd = voice_control(text)
            print(my_cmd)
            if my_cmd != None:
                wasd_interface._drive_cmd(ord(my_cmd))
                time.sleep(0.2)
            response = respond(text)
            print(response)
            for sentence in response.split('.'):
                if sentence != '':
                    enqueue_text(sentence)
                    time.sleep(0.1)
            if text == 'Ate' or text =='ate':
                break
